...
export const ThreeJSOptimizer: React.FC<ThreeJSOptimizerProps> = ({ children }) => {
  ...
  const [qualityState, setQualityState] = useState<{
    ...
    qualityPresets: null,
    currentQuality: 'high', // Start with high and let the system adjust
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      // Add quality indicator to the corner
      ...
      indicator.textContent = `Quality: ${qualityState.currentQuality}`;
      ...
      const updateIndicator = () => {
        ...
        // Add quality-specific styles
        if (qualityState.currentQuality === 'ultra') {
          indicator.style.backgroundColor = 'rgba(148, 0, 211, 0.7)'; // Purple for ultra
        } else if (qualityState.currentQuality === 'high') {
          indicator.style.backgroundColor = 'rgba(0, 128, 0, 0.7)'; // Green for high
        } else if (qualityState.currentQuality === 'medium') {
          indicator.style.backgroundColor = 'rgba(255, 140, 0, 0.7)'; // Orange for medium
        } else {
          indicator.style.backgroundColor = 'rgba(255, 0, 0, 0.7)'; // Red for low
        }
      };
      
      // Listen for quality changes
      const handleQualityChanged = () => updateIndicator();
      window.addEventListener('quality-changed', handleQualityChanged);
      ...
      return () => {
        ...
        window.removeEventListener('quality-changed', handleQualityChanged);
      };
    }
